...
//===============心知天气私钥===================//
String My_Key = "SjoXZqbQ5LegAZo_6";//需要自己注册
...
const char* ssid     = "muchen";      // 连接WiFi名需要连接的WiFi名填入引号中
const char* password = "12345678";   // 连接WiFi密码需要连接的WiFi密码填入引号中

//===============为了方便访问，配置为固定IP，需要根据自己路由器调整===================//

IPAddress local_IP(172,20,10,10);    // 设置IP192,168,171, 100    172,20,10,10
IPAddress gateway(172,20,10,1);     // 设置网关192,168,171, 70    172,20,10,1
IPAddress subnet(255, 255, 255, 0);   // 设置子网掩码
IPAddress dns(8,8,8,8);            // 设置局域网DNS
...
void setup(void)
{
  ...
  display.drawStringMaxWidth(0, 36, 128, "muchen");//英文的muchen可以直接显示
//  display.drawXbm(0 , 25, 16, 16, mu_);//显示“笨”//汉字的SSID需要处理后显示
//  display.drawXbm(16, 25, 16, 16, chen);//显示“鸟”
//  display.drawStringMaxWidth(0 , 25, 16,"muchen");//显示muchen
  ...
  display.drawStringMaxWidth(0, 36, 128, "muchen");//字符的muchen可以直接显示
//  display.drawXbm(0 , 16, 16, 16, mu);//显示“笨”//汉字的SSID需要处理后显示
//  display.drawXbm(16, 16, 16, 16, chen);//显示“鸟”
//  display.drawStringMaxWidth(0 , 25, 16,  "muchen");//显示muchen
